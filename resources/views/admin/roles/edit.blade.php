...
<div class="mx-auto">
    ...
    <div class="mb-8 flex items-center justify-between">
        <div>
            <h3 class="text-2xl font-bold text-gray-900">Edit Role: {{ $role->name }}</h3>
            <p class="text-sm text-gray-500 mt-1">Define a new access level for the system</p>
        </div>
        <a href="{{ route('admin.roles.index') }}"
           class="inline-flex items-center justify-center px-4 py-2.5 text-sm font-medium text-gray-600 dark:text-gray-300 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg hover:bg-gray-600 transition-colors shadow-sm">
        ...
        <form action="{{ route('admin.roles.update', $role->id) }}" method="POST">
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="slug" id="slug" value="{{ $role->slug }}" required
                           {{ in_array($role->slug, ['admin', 'customer']) ? 'readonly' : '' }}
                           placeholder="e.g. manager, cashier"
                           class="block w-full rounded-lg border border-gray-200 bg-gray-50 text-gray-900 placeholder-gray-400
                                  px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-colors {{ in_array($role->slug, ['admin', 'customer']) ? 'cursor-not-allowed opacity-60' : '' }}">
                    
                    @if(in_array($role->slug, ['admin', 'customer']))
                        <p class="mt-1.5 text-xs text-amber-600 font-medium">
                            <i class="fa-solid fa-triangle-exclamation mr-1"></i>
                            This is a system role. The slug cannot be modified to prevent access issues.
                        </p>
                    @else
                        <p class="mt-1.5 text-[15px] text-gray-400 italic">Unique identifier used by the system (lowercase, no spaces)</p>
                    @endif
                ...
                <div>
                   ...
                   <textarea name="description" id="description" rows="5"
                            ...
                            class="block w-full rounded-lg border border-gray-200 bg-gray-50 text-gray-900 placeholder-gray-400
                                px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-colors">{{ $role->description }}</textarea>
                    @error('description')
                        <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
