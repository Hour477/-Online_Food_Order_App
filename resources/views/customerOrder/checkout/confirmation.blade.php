...
<div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
    ...
    <p class="text-ink/50 mb-8">
        Thank you! Your order <span class="font-bold text-brand-400">#{{ $order->order_no }}</span> has been confirmed and is being prepared.
    </p>
    ...
    <div class="bg-white rounded-2xl p-6 shadow-sm border border-brand-50 text-left mb-8">
        ...
        <div class="space-y-3 mb-4">
            ...
            <div class="flex items-center gap-3">
                ...
                @endif
                <div class="flex-1">
                    <p class="text-sm font-semibold text-ink">{{ $item->menuItem->name }}</p>
                    <p class="text-xs text-ink/50">x{{ $item->quantity }}</p>
                </div>
                <span class="text-sm font-bold text-brand-500">${{ number_format($item->price * $item->quantity, 2) }}</span>
            </div>
            @endforeach
        </div>
        ...
        <div class="border-t border-brand-100 pt-4 space-y-1.5 text-sm">
            ...
            <div class="flex justify-between text-ink/60"><span>Tax (10%)</span><span>${{ number_format($order->tax, 2) }}</span></div>
            <div class="flex justify-between font-bold text-ink text-base pt-1 border-t border-brand-100 mt-1">
                <span>Total</span><span class="text-brand-500">${{ number_format($order->total_amount, 2) }}</span>
            </div>
        </div>

        <div class="mt-4 pt-4 border-t border-brand-100 text-sm">
            <p class="text-xs text-ink/40 uppercase tracking-wide">Delivery Info</p>
            <p class="font-medium text-ink mt-0.5 whitespace-pre-line">{{ $order->notes }}</p>
        </div>
    </div>

    <div class="flex flex-col sm:flex-row gap-3 justify-center">
        <a href="{{ route('customerOrder.orders.history') }}" class="btn-primary inline-flex items-center justify-center gap-2 px-6 py-3 rounded-full font-medium text-sm shadow">
            <i class="fa-solid fa-receipt"></i> View Order History
        </a>
        <a href="{{ route('customerOrder.menu.index') }}" class="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-full font-medium text-sm border-2 border-brand-200 hover:border-brand-400 transition">
