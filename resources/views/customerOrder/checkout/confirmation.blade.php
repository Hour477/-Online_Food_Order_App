...
<div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
    ...
    <h1 class="text-4xl font-bold text-gray-900 mb-2 battambang-bold">{{ __('app.order_placed') }}</h1>
    <p class="text-gray-500 mb-8 battambang-regular">
    ...
    <div class="bg-amber-50 border border-amber-200 rounded-xl p-5 mb-8 flex items-center gap-4">
        ...
        <div class="text-left">
            <p class="text-xs text-gray-500 uppercase tracking-wide font-semibold battambang-regular">{{ __('app.estimated_delivery') }}</p>
            <p class="text-2xl font-bold text-amber-600 battambang-bold">30-45 {{ __('app.minutes') }}</p>
        </div>
    </div>

    <!-- Order Details -->
    <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-200 text-left mb-8">
        <h2 class="font-bold text-lg text-gray-900 mb-4 battambang-bold">{{ __('app.order_details') }}</h2>
        ...
        <div class="border-t border-gray-200 pt-4 space-y-1.5 text-sm battambang-regular">
        ...
        <div class="mt-4 pt-4 border-t border-gray-200 text-sm">
            <p class="text-xs text-gray-400 uppercase tracking-wide battambang-regular">{{ __('app.delivery_info') }}</p>
            <p class="font-medium text-gray-900 mt-0.5 whitespace-pre-line">{{ $order->notes }}</p>
        </div>
    </div>

    <div class="flex flex-col sm:flex-row gap-3 justify-center">
        <a href="{{ route('customerOrder.orders.history') }}" class="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-lg font-medium text-sm bg-amber-600 text-white hover:bg-amber-700 transition battambang-regular">
            <i class="fa-solid fa-receipt"></i> {{ __('app.view_order_history') }}
        </a>
        <a href="{{ route('customerOrder.menu.index') }}" class="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-lg font-medium text-sm border-2 border-gray-300 hover:border-gray-400 transition text-gray-700 battambang-regular">
